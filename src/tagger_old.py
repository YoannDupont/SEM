...
from src.pretreatment.addInformations1 import addInformations as addInformations1
...
def tagger(configfile):
    ...
    if os.path.isdir(in_file):
        ...
        in_file.sort()
        in_file[:] = [os.path.join(folder, elt) for elt in in_file]
    ...
    for infile in in_file:
        ...
        if segment:
            ...
            segmented_file = os.path.join(outdir, os.path.basename(infile) + u".segment")
        ...
        if (code & POS) == POS:
            informed = os.path.join(outdir, os.path.basename(infile) + u".informed")
            wapiti_in = informed
            wapiti_out = os.path.join(outdir, os.path.basename(infile) + u".wapiti")
            
            # two variants :
            #   | LeFFF is file : canon information adding
            #   | LeFFF is directory : alternative information adding
            if lefff_pickled and os.path.isdir(lefff_pickled):
                if not quiet:
                    log(u'Alternative !\n')
                cont_tag = False
                for tag in pos_tags:
                ...
                multi  = None
                if cont_tag:
                    single = os.path.join(lefff_pickled, u"single")
                    multi  = os.path.join(lefff_pickled, u"multi")
                else:
                    single = os.path.join(lefff_pickled, u"mwe")
                wapiti_in = os.path.join(outdir, os.path.basename(infile) + ".lefff")
                addInformations1(segmented_file, wapiti_in,
                        0, 0, single, multi,
            ...
            Wapiti.label(wapiti_in, models[0], output=wapiti_out)
            ...
            postfile      = os.path.join(outdir, os.path.basename(infile) + ".POS")
        ...
        if (code & CHUNK) == CHUNK:
            ...
            if index == 1:
                outfile = os.path.join(outdir, os.path.basename(outfile) + u".CHUNK")
            else:
                outfile = os.path.join(outdir, os.path.basename(infile) + u".CHUNK")

            Wapiti.label(postfile, models[index], outfile)
