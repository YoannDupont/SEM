...
from getterfeatures     import DEFAULT_GETTER, DictGetterFeature
from dictionaryfeatures import MultiwordDictionaryFeature, MapperFeature
...
class OntologyFeature(Feature):
    def __init__(self, path, x2f, order=".order", ambiguous=False, *args, **kwargs):
        ...
        for name in self.order:
            ...
            if not (self.features[-1].is_boolean or self.features[-1].is_sequence or isinstance(self.features[-1], MapperFeature) or (isinstance(self.features[-1], TriggeredFeature) and isinstance(self.features[-1].operation, MapperFeature))):
