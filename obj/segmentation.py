...
class Segmentation(object):
    ...
    def isSentenceDot(self, the_line, token, current_position):
        ...
        if token[:-1].isdigit():
            ...
            EOS = (current_position == (len(the_line) - 2)) and (the_line[current_position + 1][0] not in self.closing_chars + u'"')
            #EOS = False
        return EOS
    ...
    def isAbbreviationBeforeNoun(self, token):
        abbr_list = [u"dr.", u"mme.", u"mmes.", u"melle.", u"melles.", u"mlle.", u"mlles.", u"m.", u"mr.", u"me.", u"mrs.", u"st."]
            ...
            for line in paragraph:
                ...
                while not EOL:
                    ...
                    if self.isChar(token):
                        ...
                        self.sentence += u" " + token[:index] + u" " + token[index:]
                        ...
                        if token == u"...":
                            ...
                            self.sentence += u" " + token[0 : index] + u" " + token[index : len(token)-1] + u" " + token[-1]
                        else:
                        ...
                        if token[:index].isdigit() and token[index+1:].isdigit():
                            ...
                            if self.isAbbreviationBeforeNoun(token[:index + 1]):
                                self.sentence += u" " + token[:index + 1] + u" " + token[index + 1:]
                            else:
